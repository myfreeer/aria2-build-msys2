...
:Init
%sh%  -lc "pacman-key --init && exit" 2>&1 >>init.log

:Chi_Mirror
for %%i in ( mirrorlist.mingw32 mirrorlist.mingw64 mirrorlist.msys) do if exist %%i copy /y %%i msys64\etc\pacman.d\

:Update
%pacman% -Syyuu --needed --noconfirm 2>&1 >>update.log

:Install
%pacman% -S --needed --noconfirm base-devel zlib-devel sqlite git unzip zip tar gmp gmp-devel libssh2 libssh2-devel openssl-devel 2>&1 >>install.log
%pacman% -Sc --noconfirm 2>&1 >>install.log

:Clone
%sh%  -lc "if [[ -d ~/aria2 ]]; then cd aria2; git pull; else git clone https://gist.github.com/a780c730b7282e090f238e8286f815f3.git aria2; cd aria2; git pull; fi" 2>&1 >>clone.log

:Build
%sh%  -lc "cd ~/aria2 && exec ./build-aria2.sh" 2>&1 >>build.log
pause